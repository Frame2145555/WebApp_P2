document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadDashboard() {
    ...
    try {
      state.results = sortResults(await VotingApp.api('/api/candidate/results'));
  ...
  async function hydrateCandidateProfile() {
    try {
      const response = await VotingApp.api(`/api/candidate/profile/${user.user_id}`);
      const latestUser = response.user || {};

      user.candidate_id = latestUser.candidate_id ?? user.candidate_id ?? null;
      user.bio = latestUser.bio ?? user.bio ?? null;
      user.profile_picture = latestUser.profile_picture ?? user.profile_picture ?? null;
      user.display_name = latestUser.display_name ?? user.display_name ?? user.username;

      VotingApp.setUser(user);
      elements.displayName.textContent = user.display_name;
      elements.summaryName.textContent = user.display_name;
      syncBioFromUser();
      renderProfilePicture(user.profile_picture);
    } catch (error) {
      if (!user.bio) {
        elements.displayBio.textContent = 'No manifesto found for this candidate yet.';
      }
    }
  }
  ...
  async function saveBio() {
    ...
    try {
      const response = await VotingApp.api('/api/candidate/manifesto', {
        method: 'POST',
        body: {
  ...
  async function saveProfilePicture() {
    ...
    try {
      const response = await VotingApp.api('/api/candidate/profile-picture', {
        method: 'POST',
        body: {
          user_id: user.user_id,
